...
#python3 LocalObj_DataPointCollection_VarLength.py /afs/inf.ed.ac.uk/user/s15/s1545529/Desktop/datastorage/LargeSlope_TimeTrack_Angle_17_26/ RollOuts_8Steps_from_3StepbeforeFail_TrackTrainingExtra_InitSet TrainingAug1Time_3StepBeforeFail_TrackTrainingExtra_InitSet OriginalForm 1.0 KeepOutlier CleanTrainingRollOuts_Extra Unseen_3StepbeforeFail_TrackingTrainingExtra_InitSet 0.06
#python3 LocalObj_DataPointCollection_VarLength.py /afs/inf.ed.ac.uk/user/s15/s1545529/Desktop/datastorage/LargeSlope_TimeTrack_Angle_17_26/ RollOuts_8Steps_from_3StepbeforeFail_TrackTrainingInit_InitSet TrainingAug1Time_3StepBeforeFail_TrackTrainingInit_InitSet OriginalForm 1.0 KeepOutlier CleanTrainingRollOuts_Init Unseen_3StepbeforeFail_TrackingTrainingInit_InitSet 0.06
...
#------------------------
#Definr RollOut Path
rolloutPath = workingDirectory + '/' + RollOutFoldername #"/CleanTrainingSetRollOuts/"
...
PreProcessingMode = sys.argv[4] #Can be 1) "OriginalForm" 2) "Standarization" 3) "MaxAbs" (lies in [-1, 1])
...
if PreProcessingMode == "OriginalForm":
    ScaleFactor = float(sys.argv[5]);      print("Scaling Factor of all quantities (Except Left and Right Swing Flag):\n", ScaleFactor)
else:
    ScaleFactor = 1.0
...
for filename in filenames:
    if ".p" in filename:#a data file
        ...
        if len(data["SingleOptResultSavings"]) == data["Num_of_Rounds"]:
            ...
            UnseenStateFilePath = unseenstate_Path + "/" + UnseenStateFilePath[idxTemp:]
            ...
            GroundTruthFilePath = groundtruth_rollout_Path + "/" + GroundTruthFilePath[idxTemp:]
            ...
            #Build the dist vec
            for roundNum in range(len(data["SingleOptResultSavings"])):#range(data["Num_of_Rounds"]):
                ...
                if UnseenStateRoundIdx + roundNum > len(GroundTruth_data["SingleOptResultSavings"]) - 1:
                    ...
                    x_ground_truth, y_ground_truth = getDataPoints(SingleOptRes = GroundTruth_Opt_Result[UnseenStateRoundIdx + roundNum], Shift_World_Frame = Shift_World_Frame_Type, ContactRepresentationType = Contact_Representation_Type, VectorScaleFactor = ScaleFactor) #InitCoM; InitSurfBorder
                else:
            ...
            if np.sum(largeError_flag) == len(largeError_flag): #Did not find small error steps until the end (most probably last few rounds)
                RecoveryTraj_Length = len(largeError_flag)
            elif largeError_flag[0] == False: #False starts from very beginning, need to add at least one round
                RecoveryTraj_Length = 1
            else:
                RecoveryTraj_Length = largeError_flag.index(False) + 1 - 1
            print("Recovery Traj Length Start from Round: ", UnseenStateRoundIdx, "with Length: ", RecoveryTraj_Length)
            #Record the value
