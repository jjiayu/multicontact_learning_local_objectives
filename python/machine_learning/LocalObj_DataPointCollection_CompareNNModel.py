...
ThresholdVec = np.concatenate((CoM_pos_thre,CoM_vel_thre,AM_thre,ContactLocation_thre), axis = None)
...
for filename in filenames:
    if ".p" in filename:#a data file
        ...
        if len(data["SingleOptResultSavings"]) == data["Num_of_Rounds"]:
            ...
            for roundNum in range(startroundNum,terminalroundNum+1):#range(data["Num_of_Rounds"]):
                ...
                if np.sum(diff_flag) > 0:
                    print("Prediction with large Error at Round: ", roundNum, "add datapoint to the dataset")
...
print("- Total Number of Raw DataPoint we have: ", success_file_num*data["Num_of_Rounds"])
