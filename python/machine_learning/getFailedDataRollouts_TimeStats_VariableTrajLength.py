...
# $3: Start Round Num
# $4: End Round Num
# $5: Round Idx when Large Slope in Horizon
...
RoundNum_LargeSlope_in_Horizon = int(sys.argv[5])
...
for filename in filenames:
    if ".p" in filename:#a data file        
        ...
        if len(data["SingleOptResultSavings"]) >= RoundNum_LargeSlope_in_Horizon + 1 - 1:
            total_file_num = total_file_num + 1

        if len(data["SingleOptResultSavings"]) >= RoundNum_LargeSlope_in_Horizon + 1 - 1:#2:
            Fail_due_time_Vec = []
            for roundIdx in range(startRoundNUm,np.min([len(data["SingleOptResultSavings"]),endRoundNum+1])):
                ...
                if total_proc_time > time_limit:
                    Fail_due_time_Vec.append(True)
                elif total_proc_time <= time_limit:
            ...
            if fail_due_to_time == True: #Failed Due to Time
                List_StepMade.append(Failed_Round_Idx + 1 - 1) #do not include the failed round
                failedIndex.append(Failed_Round_Idx)
                print("Process: ",filename)
                print("Failed (Due to Large Computation Time) at round: ", Failed_Round_Idx) 
                print(data["SingleOptResultSavings"][Failed_Round_Idx]["SolverStats"]["t_wall_total"])
                print("Terrain Model Path: ",data["TerrainModelPath"])

            elif fail_due_to_time == False: #Not failing due to time
                if (len(data["SingleOptResultSavings"]) >= startRoundNUm + 1 - 1) and (len(data["SingleOptResultSavings"]) < endRoundNum+1): #But fail due to convergence
                    failedIndex.append(len(data["SingleOptResultSavings"]) + 1 - 1) #one step more than steps made, but index -1 
                    List_StepMade.append(len(data["SingleOptResultSavings"]))
                    #Print failed round info
                    print("Process: ",filename)
                    print("Failed at round (Due to Convergence): ", len(data["SingleOptResultSavings"]))
                    print("Terrain Model Path: ",data["TerrainModelPath"])
                else: #We get successful rollout
                    success_file_num = success_file_num + 1
                    List_StepMade.append(endRoundNum+1)
                    #List_StepMade.append(len(data["SingleOptResultSavings"]))
            ...
            #Get End CoM
            end_CoM = data["SingleOptResultSavings"][List_StepMade[-1]-1]["opt_res"][endCoM_var_idx]
            #Get dist
...
successrate = success_file_num/(total_file_num)*100.0
...
computation_time_upperlower_limit = 1.96*computation_time_std/np.sqrt(len(computation_time_list))

print("   -Confidence Interval on Computation time:")
print("       -Mean: ", np.round(computation_time_mean,3))
print("       -Upper and Lower Limit: ", np.round(computation_time_upperlower_limit,3))
