...
for filename in filenames:
    if ".p" in filename:  # a data file
        ...
        if len(data["SingleOptResultSavings"]) > 2:
            ...
            for roundNum in range(startingIdx, np.min([len(data["SingleOptResultSavings"]), EndIdx+1])):
                ...
                timing_lv1_fixed = singleOptResult["opt_res"][decision_var_index_level1["Ts"]
                                                              [0]:decision_var_index_level1["Ts"][1]+1]
                ...
                if data["NumLookAhead"] > 1:
                    ...
                    timing_lv2_fixed = x_opt_level2[decision_var_index_level2["Ts"]
                                                    [0]:decision_var_index_level2["Ts"][1]+1]

                    for timingidx in range(len(timing_lv2_fixed)/3):
                        InitialDoubleSupport_List.append(
                            timing_lv1_fixed[3*timingidx+0])
                        SingleSupport_List.append(
                            timing_lv1_fixed[3*timingidx+1])
                        DoubleSupport_List.append(
                            timing_lv1_fixed[3*timingidx+2])
