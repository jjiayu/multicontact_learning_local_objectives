...
for TrackingExp_filename in TrackingExpfilenames:
    ...
    if ".p" in TrackingExp_filename:#a data file
        ...
        for roundNum in range(1,len(TrackingExp_data["SingleOptResultSavings"])): #NOTE: Skip Round 0 Failures that we dont have any savings
            ...
            TrackingExp_InitConfig, TrackingExp_TerminalConfig = getInitConfig_in_GlobalFrame_from_SingleOptResult(SingleOptRes=TrackingExp_SingleOptRes)
            GroundTruth_InitConfig, GroundTruth_TerminalConfig = getInitConfig_in_GlobalFrame_from_SingleOptResult(SingleOptRes=GroundTruth_SingleOptRes)
            ...
            InitState_Diff = np.array([TrackingExp_InitConfig["x_init"]-GroundTruth_InitConfig["x_init"],
                                       TrackingExp_InitConfig["y_init"]-GroundTruth_InitConfig["y_init"],
                                       TrackingExp_InitConfig["z_init"]-GroundTruth_InitConfig["z_init"],
                                       TrackingExp_InitConfig["xdot_init"]-GroundTruth_InitConfig["xdot_init"],
                                       TrackingExp_InitConfig["ydot_init"]-GroundTruth_InitConfig["ydot_init"],
                                       TrackingExp_InitConfig["zdot_init"]-GroundTruth_InitConfig["zdot_init"],
                                       TrackingExp_InitConfig["Lx_init"]-GroundTruth_InitConfig["Lx_init"],
                                       TrackingExp_InitConfig["Ly_init"]-GroundTruth_InitConfig["Ly_init"],
                                       TrackingExp_InitConfig["Lz_init"]-GroundTruth_InitConfig["Lz_init"],
                                       TrackingExp_InitConfig["PLx_init"]-GroundTruth_InitConfig["PLx_init"],
                                       TrackingExp_InitConfig["PLy_init"]-GroundTruth_InitConfig["PLy_init"],
                                       TrackingExp_InitConfig["PLz_init"]-GroundTruth_InitConfig["PLz_init"],
                                       TrackingExp_InitConfig["PRx_init"]-GroundTruth_InitConfig["PRx_init"],
                                       TrackingExp_InitConfig["PRy_init"]-GroundTruth_InitConfig["PRy_init"],
            ...
            if Diff_Boolean_vec.sum() > 0:
                ...
                if getUnseenStatefrom == "Failed":
                    if FailedRound == True:
                        pickle.dump(UnseenState, open(UnseenStateStoragePath + '/' + TrackingExp_filename[:-2] + '_UnseenState' + str(UnseenStateCnt) +".p", "wb"))    #Save Data
                elif getUnseenStatefrom == "All":
                    pickle.dump(UnseenState, open(UnseenStateStoragePath + '/' + TrackingExp_filename[:-2] + '_UnseenState' + str(UnseenStateCnt) +".p", "wb"))    #Save Data
                else:
                    raise Exception("Unknow Flag of getting unseen state from what kind of rollouts")
        ...
        if np.sum(np.array(divergingRound_indicator)) > 0.0: #Note fail from the first step
            firstDivergingRound = divergingRound_indicator.index(1) #ignore the first element to avoid failures at round 0
