...
if os.path.isdir(ExtractedEnv_Path):
    raise Exception(
...
for filename in filenames:
    if ".p" in filename:  # a data file
        ...
        # made the first 2 (quasi-flat) steps but failed eventually
        if len(data["SingleOptResultSavings"]) > 2 and not (len(data["SingleOptResultSavings"]) == data["Num_of_Rounds"]):
            Env_File_Path = data["TerrainModelPath"]
            Env_File_Name_Idx = Env_File_Path.find("Group")
            # Cope p file
            src_path = LookAhead_NSteps_Path + '/' + \
                Env_File_Path[Env_File_Name_Idx:]
            dst_path = ExtractedEnv_Path
            print("Copy: ", src_path)
            shutil.copy(src_path, dst_path)
            # Cope txt File
            src_path = LookAhead_NSteps_Path + '/' + \
                Env_File_Path[Env_File_Name_Idx:][:-2] + ".txt"
            dst_path = ExtractedEnv_Path
            print("Copy: ", src_path)
            shutil.copy(src_path, dst_path)
