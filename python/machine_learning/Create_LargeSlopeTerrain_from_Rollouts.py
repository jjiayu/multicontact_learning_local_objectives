...
if os.path.isdir(LargeSlopeFolder):
    raise Exception(
...
print("Our Terrain start from Step: ", StepIdx_for_getting_StartofTerrain,
      "Note: We land on this step, but the start of the terrain is actually the initial patches of this step")

# Large Slope Modification Idx
LargeSlopPatchIdx_relative_to_theFirstStep = int(sys.argv[5])
print("Index of the Large Slope Patch (at which step we have the large slope?): ",
      LargeSlopPatchIdx_relative_to_theFirstStep)
...
for Rubbles_Rollout_filename in Rubbles_RollOut_filenames:
    ...
    if ".p" in Rubbles_Rollout_filename:  # a data file
        ...
        print("Process Rubble Rollout File (Computed from full NLP): ",
              Rubbles_Rollout_filename)
        ...
        if len(Rubbles_RollOut_data["SingleOptResultSavings"]) == Rubbles_RollOut_data["Num_of_Rounds"]:
            ...
            SingleOptResult_for_StartofTerrain = Rubbles_RollOut_data[
                "SingleOptResultSavings"][roundIdx_of_interst]

            # Get Init and Terminal Config of the plan
            Rubbles_RollOut_InitConfig, Rubbles_RollOut_TerminalConfig = getInitConfig_in_GlobalFrame_from_SingleOptResult(
                SingleOptRes=SingleOptResult_for_StartofTerrain)

            # Save Unseen State (from local obj tracking exp) and Environment Model
            LargeSlopeSetup = {}
            LargeSlopeSetup["Rubbles_RollOut_filepath"] = Rubbles_RollOut_Path + \
                '/' + Rubbles_Rollout_filename
            ...
            # Save Files
            pickle.dump(LargeSlopeSetup, open(LargeSlopeFolder + '/' + Rubbles_Rollout_filename[:-2] + '_start_' + str(StepIdx_for_getting_StartofTerrain) + "_large_" + str(
                LargeSlopPatchIdx_relative_to_theFirstStep) + "_" + rotationType + "_angle_" + str(np.round(useful_Info["rotation_angle"]/np.pi*180, 1)) + ".p", "wb"))  # Save Data
            print(" ")
