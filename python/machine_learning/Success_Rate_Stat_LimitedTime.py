        ...
        if len(data["SingleOptResultSavings"]) >= 2:  # Overcome the first two steps
            ...
            if len(data["SingleOptResultSavings"]) == data["Num_of_Rounds"]:
                total_steps_attempted = total_steps_attempted + \
                    data["Num_of_Rounds"] - 2
                total_steps_made = total_steps_made + \
                    len(data["SingleOptResultSavings"]) - 2
            # did not complete the whole journey
            elif not (len(data["SingleOptResultSavings"]) == data["Num_of_Rounds"]):
                total_steps_attempted = total_steps_attempted + \
                    len(data["SingleOptResultSavings"]) - 1 + \
                    1 + 1 - 2  # remove the first two steps
...
print("Total Number of Steps Failed to compute within the Time Budget: ",
      total_steps_failed)
print("    - Percentage: ",
