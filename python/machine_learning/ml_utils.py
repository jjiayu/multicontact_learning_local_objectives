...
def getDataPoints(SingleOptRes=None, Shift_World_Frame=None, ContactRepresentationType=None, VectorScaleFactor=1, Phase_Duration_in_Data = False):
    ...
    InitConfig, TerminalConfig = getInitTerminalConfig(
        SingleOptRes=SingleOptRes, Shift_World_Frame=Shift_World_Frame)
    ...
    if Shift_World_Frame == None:  # No transformation to local frames, then full input vector
        ...
        # Swing Left Foot
        if (InitConfig["LeftSwingFlag"] == 1) and (InitConfig["RightSwingFlag"] == 0):
            contact_before_swing_X = InitConfig["PLx_init"]
            contact_before_swing_Y = InitConfig["PLy_init"]
            contact_before_swing_Z = InitConfig["PLz_init"]
        # Swing Right Foot
        elif (InitConfig["LeftSwingFlag"] == 0) and (InitConfig["RightSwingFlag"] == 1):
            contact_before_swing_X = InitConfig["PRx_init"]
            contact_before_swing_Y = InitConfig["PRy_init"]
    ...
    if ContactRepresentationType == None:
        raise Exception(
            "Unknown Contact Location Representation Type (ContactRepresentationType)")
    ...
    # --------------------------
    # Scale Vectors
    x = x*VectorScaleFactor
    y = y*VectorScaleFactor
