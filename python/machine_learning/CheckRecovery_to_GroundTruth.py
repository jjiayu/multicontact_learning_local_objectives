...
nbrs = NearestNeighbors(n_neighbors=1, algorithm='brute', radius= 0.05).fit(x_train)
...
for roundIdx in range(10):
    ...
    x_recompute, y_recompute = getDataPoints(SingleOptRes = Recompute_Opt_Result[roundIdx], Shift_World_Frame = Shift_World_Frame_Type, ContactRepresentationType = Contact_Representation_Type, VectorScaleFactor = 1.0) #InitCoM; InitSurfBorder
    ...
    x_ground_truth, y_ground_truth = getDataPoints(SingleOptRes = GroundTruth_Opt_Result[UnseenRoundIdx+roundIdx], Shift_World_Frame = Shift_World_Frame_Type, ContactRepresentationType = Contact_Representation_Type, VectorScaleFactor = 1.0) #InitCoM; InitSurfBorder
    #print("Difference: ", np.linalg.norm(x_recompute - x_ground_truth))
    print("Differenc (Neighbor): ", np.linalg.norm(x_recompute - x_train[dist_min_idx,:]), "; Agains Ground Truth: ", np.linalg.norm(x_recompute - x_ground_truth))

#Show Figures
fig=plt.figure();   ax = Axes3D(fig)  
viz.DisplayResults_Not_Show(TerrainModel = Recompute_data["TerrainInfo"], SingleOptResult = None, AllOptResult = Recompute_data["SingleOptResultSavings"], fig = fig, ax = ax)
