...
#GroupName=Group20 && longjob -28day -c "nice bash server_NN_trackExp_LargeSlope_StartwithLookUp_TrainingSet_SameGroupPrefix.sh LargeSlope_TimeTrack_Angle_17_26 NN_TrackTrainingExtra_LargeSlope_Aug_1Time_EarlyStopping CleanTrainingRollOuts_Extra $GroupName $GroupName /afs/inf.ed.ac.uk/user/s15/s1545529/Desktop/datastorage/LargeSlope_TimeTrack_Angle_17_26/ML_Models/NN_Model_Aug_1Time_EarlyStopping 8 2"
...
for i in $4_*.p; do #Random loop control, just loop over all the files
   ...
   #Or we use the first few files from the list
    ...
    python3 -W ignore /afs/inf.ed.ac.uk/user/s15/s1545529/Desktop/multicontact_learning_local_objectives/python/rhp_plan/rhp_gen_few_lookupobj.py \
