...
#GroupName=Group20 && longjob -28day -c "nice bash server_NNRelaxed_trackExp_TrainingSet_SameGroupPrefix.sh Rubbles_RegretOneStep NN_Relax_TrackTraining_All_Aug_1StepBeforeFail_1Time CleanTrainingSetRollOuts_All_Backup $GroupName $GroupName /afs/inf.ed.ac.uk/user/s15/s1545529/Desktop/datastorage/Rubbles_RegretOneStep/ML_Models/NN_Model_Valid_OriginalForm_Aug_1StepBeforeFail_1Time"
...
for i in $4_*.p; do #Random loop control, just loop over all the files
   ...
   #Or we use the first few files from the list
    ...
    python3 -W ignore /afs/inf.ed.ac.uk/user/s15/s1545529/Desktop/multicontact_learning_local_objectives/python/rhp_plan/rhp_gen_relaxed.py \
