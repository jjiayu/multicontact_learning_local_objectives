...
def drawFootPatch(P = None, P_TangentX = None, P_TangentY = None, line_color = None, LineWidth = 2, ax = None):
    ...
    ax.plot(cx, cy, cz, color = line_color, linewidth = LineWidth)
...
def labelSurface(Sl0surf = None, Sr0surf = None, ContactSurfs = None, fig = None, ax = None):
    ...
    Sl0_center_x, Sl0_center_y, Sl0_center_z = geometric_utils.getCenter(Surface = Sl0surf)
    #Label Init Left Patch Center
    ax.text(Sl0_center_x,Sl0_center_y,Sl0_center_z,'SL0')

    #Get Center of Init Right Patch
    Sr0_center_x, Sr0_center_y, Sr0_center_z = geometric_utils.getCenter(Surface = Sr0surf)
...
def drawSingleOptTraj(optResult = None, fig= None, ax = None, FootMarkerSize = 4):
    ...
    #Plot CoM Traj
    ax.plot3D(x_lv1_res,y_lv1_res,z_lv1_res,color='blue', linestyle='dashed', linewidth=2, markersize=12)
    #Plot Initial Contact Location
    ax.scatter(optResult["PLx_init"], optResult["PLy_init"], optResult["PLz_init"], c='r', marker='o', linewidth = FootMarkerSize) 
    ax.scatter(optResult["PRx_init"], optResult["PRy_init"], optResult["PRz_init"], c='b', marker='o', linewidth = FootMarkerSize) 
    #Draw Initial Contact Patches
    ax = drawFootPatch(P = np.array([optResult["PLx_init"], optResult["PLy_init"], optResult["PLz_init"]]), 
                       P_TangentX = optResult["PL_init_TangentX"], P_TangentY = optResult["PL_init_TangentY"], line_color = 'r', ax = ax)
    ax = drawFootPatch(P = np.array([optResult["PRx_init"], optResult["PRy_init"], optResult["PRz_init"]]), 
                       P_TangentX = optResult["PR_init_TangentX"], P_TangentY = optResult["PR_init_TangentY"], line_color = 'b', ax = ax)
    ...
    if optResult["LeftSwingFlag"] == 1:
        ...
        StepColor = 'b'
    #plot location
    ax.scatter(px_lv1_res, py_lv1_res, pz_lv1_res, c=StepColor, marker='o', linewidth = FootMarkerSize)    
    ...
    ax = drawFootPatch(P = np.concatenate((px_lv1_res, py_lv1_res, pz_lv1_res),axis = None), 
                       P_TangentX = optResult["SurfTangentsX"][0], P_TangentY = optResult["SurfTangentsY"][0], line_color = StepColor, ax = ax)
    ...
    if var_idx_lv2: #If we have the second level, get the optimization result
        #CoM x, y, z
        x_lv2_res = np.array(x_opt_lv2[var_idx_lv2["x"][0]:var_idx_lv2["x"][1]+1])
        y_lv2_res = np.array(x_opt_lv2[var_idx_lv2["y"][0]:var_idx_lv2["y"][1]+1])
        z_lv2_res = np.array(x_opt_lv2[var_idx_lv2["z"][0]:var_idx_lv2["z"][1]+1])
        #Contact locations
        px_lv2_res = np.array(x_opt_lv2[var_idx_lv2["px"][0]:var_idx_lv2["px"][1]+1])
        py_lv2_res = np.array(x_opt_lv2[var_idx_lv2["py"][0]:var_idx_lv2["py"][1]+1])
        pz_lv2_res = np.array(x_opt_lv2[var_idx_lv2["pz"][0]:var_idx_lv2["pz"][1]+1])

        #Plot CoM Traj
        ax.plot3D(x_lv2_res,y_lv2_res,z_lv2_res,color='green', linestyle='dashed', linewidth=2, markersize=12)
        ...
        if optResult["LeftSwingFlag"] == 1:
            StepColor = 'b'
            ax.scatter(px_lv2_res[0::2], py_lv2_res[0::2], pz_lv2_res[0::2], c=StepColor, marker='o', linewidth = FootMarkerSize) 
            StepColor = 'r'
            ax.scatter(px_lv2_res[1::2], py_lv2_res[1::2], pz_lv2_res[1::2], c=StepColor, marker='o', linewidth = FootMarkerSize) 

            #Get Contact location and parameters for left and right foot
            px_lv2_right_foot = px_lv2_res[0::2];    py_lv2_right_foot = py_lv2_res[0::2];    pz_lv2_right_foot = pz_lv2_res[0::2]
            lv2_ContactTengentX_right_foot = lv2_ContactTengentX[0::2]; lv2_ContactTengentY_right_foot = lv2_ContactTengentY[0::2]

            px_lv2_left_foot = px_lv2_res[1::2];    py_lv2_left_foot = py_lv2_res[1::2];    pz_lv2_left_foot = pz_lv2_res[1::2]
            lv2_ContactTengentX_left_foot = lv2_ContactTengentX[1::2]; lv2_ContactTengentY_left_foot = lv2_ContactTengentY[1::2]
            ...
            #Get Contact location and parameters for left and right foot
            px_lv2_left_foot = px_lv2_res[0::2];    py_lv2_left_foot = py_lv2_res[0::2];    pz_lv2_left_foot = pz_lv2_res[0::2]
            lv2_ContactTengentX_left_foot = lv2_ContactTengentX[0::2]; lv2_ContactTengentY_left_foot = lv2_ContactTengentY[0::2]

            px_lv2_right_foot = px_lv2_res[1::2];    py_lv2_right_foot = py_lv2_res[1::2];    pz_lv2_right_foot = pz_lv2_res[1::2]
            lv2_ContactTengentX_right_foot = lv2_ContactTengentX[1::2]; lv2_ContactTengentY_right_foot = lv2_ContactTengentY[1::2]
...
def DrawAllExecutionHorizon(allOptResult = None, fig = None, ax = None, FootMarkerSize = 4):
    #Get First Level Result
    var_idx_lv1 = allOptResult[0]["var_idx"]["Level1_Var_Index"] #Get from the first elements info

    #Draw Intial Footstep Location and contact patches
    #For the initial left
    ax.scatter(allOptResult[0]["PLx_init"], allOptResult[0]["PLy_init"], allOptResult[0]["PLz_init"], c='r', marker='o', linewidth = FootMarkerSize) 
    ax = drawFootPatch(P = np.array([allOptResult[0]["PLx_init"], allOptResult[0]["PLy_init"], allOptResult[0]["PLz_init"]]), 
                       P_TangentX = allOptResult[0]["PL_init_TangentX"], P_TangentY = allOptResult[0]["PL_init_TangentY"], line_color = 'r', ax = ax)
    #For the initial right
    ax.scatter(allOptResult[0]["PRx_init"], allOptResult[0]["PRy_init"], allOptResult[0]["PRz_init"], c='b', marker='o', linewidth = FootMarkerSize) 
    ax = drawFootPatch(P = np.array([allOptResult[0]["PRx_init"], allOptResult[0]["PRy_init"], allOptResult[0]["PRz_init"]]), 
                       P_TangentX = allOptResult[0]["PR_init_TangentX"], P_TangentY = allOptResult[0]["PR_init_TangentY"], line_color = 'b', ax = ax)
    ...
    for roundNum in range(len(allOptResult)):
        ...
        if roundNum%2 == 0: #Even number of steps
            ...
            #FootStep Color
            if allOptResult[0]["LeftSwingFlag"] == 1: #Even round, and the first step swing left
                P_Color = "red"
            elif allOptResult[0]["RightSwingFlag"] == 1: #Even round, and the first step swing right
                P_Color = "blue"
            ...
            #FootStep color
            if allOptResult[0]["LeftSwingFlag"] == 1: #odd round, and the first step swing left
                P_Color = "blue"
            elif allOptResult[0]["RightSwingFlag"] == 1: #odd round, and the first step swing roight
                P_Color = "red"
        ...
        #Plot CoM
        ax.plot3D(x_lv1_res,y_lv1_res,z_lv1_res,color=CoM_Color, linestyle='dashed', linewidth=2, markersize=12)
        #Plot Step Location
        ax.scatter(px_lv1_res,py_lv1_res,pz_lv1_res, c=P_Color, marker='o', linewidth = FootMarkerSize) 
        #Draw Contact Patch
        ax = drawFootPatch(P = np.concatenate((px_lv1_res, py_lv1_res, pz_lv1_res),axis = None), 
                           P_TangentX = Contact_TangentX, P_TangentY = Contact_TangentY, line_color = P_Color, ax = ax)

    #Set xlim
    ax.set_xlim3d(allOptResult[0]["opt_res"][var_idx_lv1["x"][0]]-0.2,allOptResult[-1]["opt_res"][var_idx_lv1["x"][-1]]+0.5)
    return ax
...
def drawInitConfig(InitConfig = None, fig = None, ax = None, color = None, LineWidth = 10):
    if color == None:
        #draw init CoM
        ax.scatter(InitConfig["x_init"], InitConfig["y_init"], InitConfig["z_init"], c='g', marker='o', linewidth = LineWidth) 
        #draw init Left contact location and patch
        ax.scatter(InitConfig["PLx_init"], InitConfig["PLy_init"], InitConfig["PLz_init"], c='r', marker='o', linewidth = LineWidth) 
        drawFootPatch(P = np.array([InitConfig["PLx_init"], InitConfig["PLy_init"], InitConfig["PLz_init"]]), P_TangentX = InitConfig["PL_init_TangentX"], P_TangentY = InitConfig["PL_init_TangentY"], line_color = 'r',ax = ax)
        #draw init Right contact location
        ax.scatter(InitConfig["PRx_init"], InitConfig["PRy_init"], InitConfig["PRz_init"], c='b', marker='o', linewidth = LineWidth) 
        drawFootPatch(P = np.array([InitConfig["PRx_init"], InitConfig["PRy_init"], InitConfig["PRz_init"]]), P_TangentX = InitConfig["PR_init_TangentX"], P_TangentY = InitConfig["PR_init_TangentY"], line_color = 'b',ax = ax)
    else:
        #draw init CoM
        ax.scatter(InitConfig["x_init"], InitConfig["y_init"], InitConfig["z_init"], c=color, marker='o', linewidth = LineWidth) 
        #draw init Left contact location and patch
        ax.scatter(InitConfig["PLx_init"], InitConfig["PLy_init"], InitConfig["PLz_init"], c=color, marker='o', linewidth = LineWidth) 
        drawFootPatch(P = np.array([InitConfig["PLx_init"], InitConfig["PLy_init"], InitConfig["PLz_init"]]), P_TangentX = InitConfig["PL_init_TangentX"], P_TangentY = InitConfig["PL_init_TangentY"], line_color = color,ax = ax)
        #draw init Right contact location
        ax.scatter(InitConfig["PRx_init"], InitConfig["PRy_init"], InitConfig["PRz_init"], c=color, marker='o', linewidth = LineWidth) 
        drawFootPatch(P = np.array([InitConfig["PRx_init"], InitConfig["PRy_init"], InitConfig["PRz_init"]]), P_TangentX = InitConfig["PR_init_TangentX"], P_TangentY = InitConfig["PR_init_TangentY"], line_color = color,ax = ax)
...
def drawLocaObj(LocalObj = None, InitConfig = None, fig = None, ax = None, LineWidth = 10):
    #draw CoM Obj
    ax.scatter(LocalObj["x_obj"],  LocalObj["y_obj"],  LocalObj["z_obj"], c= 'c', marker='o', linewidth = LineWidth) 
    #draw Contact location and foot patch
    ax.scatter(LocalObj["Px_obj"], LocalObj["Py_obj"], LocalObj["Pz_obj"], c='c', marker='o', linewidth = LineWidth) 
    ax = drawFootPatch(P = np.concatenate(([LocalObj["Px_obj"], LocalObj["Py_obj"], LocalObj["Pz_obj"]]),axis=None), P_TangentX = InitConfig["SurfTangentsX"][0], P_TangentY = InitConfig["SurfTangentsY"][0], line_color = 'c',ax = ax)
...
def drawTerminalConfig(TerminalConfig = None, InitConfig = None, fig = None, ax = None, color = 'k', LineWidth = 5):
    #draw Terminal CoM
    ax.scatter(TerminalConfig["x_end"],  TerminalConfig["y_end"],  TerminalConfig["z_end"], c= color, marker='o', linewidth = LineWidth) 
    #draw Contact location and foot patch
    ax.scatter(TerminalConfig["Px"], TerminalConfig["Py"], TerminalConfig["Pz"], c=color, marker='o', linewidth = LineWidth) 
    ax = drawFootPatch(P = np.concatenate(([TerminalConfig["Px"], TerminalConfig["Py"], TerminalConfig["Pz"]]),axis=None), P_TangentX = InitConfig["SurfTangentsX"][0], P_TangentY = InitConfig["SurfTangentsY"][0], line_color = color,ax = ax)
    ...
    #Display Optimization Result for all rounds/steps
        ...
        if AllOptResult:
            ...
            ax = drawTerrain(Sl0surf      = TerrainModel["InitLeftSurfVertice"], Sr0surf = TerrainModel["InitRightSurfVertice"], 
                            ContactSurfs = TerrainModel["ContactSurfsVertice"], 
                            printTerrainVertice = False, fig = fig, ax = ax)
            ax = DrawAllExecutionHorizon(allOptResult = AllOptResult, fig = fig, ax = ax)
...
def PrintSingleOptTraj(SingleOptResult = None, LocalObjSettings = None, LocalObj = None):
    if SingleOptResult != None: #print single Optimization result
        ...
        if LocalObjSettings["local_obj_tracking_type"] != None:
            ...
            print("- Local Obj CoM Position: x = ", str(LocalObj["x_obj"]), ", y = ", str(LocalObj["y_obj"]), ", z = ", str(LocalObj["z_obj"]))
            print("- Planned CoM Position:   x = ", str(x_res_lv1[-1]), ", y = ", str(y_res_lv1[-1]), ", z = ", str(z_res_lv1[-1]))
            print("- Local Obj and Plan CoM Diff (Abusolute Value): x = ", str(np.absolute(LocalObj["x_obj"]-x_res_lv1[-1])), "y = ", str(np.absolute(LocalObj["y_obj"]-y_res_lv1[-1])), "z = ", str(np.absolute(LocalObj["z_obj"]-z_res_lv1[-1])))

            print("-----Terminal CoM Velocity--------")
            print("- Local Obj CoM Velocity: xdot = ", str(LocalObj["xdot_obj"]), ", ydot = ", str(LocalObj["ydot_obj"]), ", zdot = ", str(LocalObj["zdot_obj"]))
            print("- Planned CoM Velocity:   xdot = ", str(xdot_res_lv1[-1]), ", ydot = ", str(ydot_res_lv1[-1]), ", zdot = ", str(zdot_res_lv1[-1]))
            print("- Local Obj and Plan CoM Diff (Abusolute Value): xdot = ", str(np.absolute(LocalObj["xdot_obj"]-xdot_res_lv1[-1])), "ydot = ", str(np.absolute(LocalObj["ydot_obj"]-ydot_res_lv1[-1])), "zdot = ", str(np.absolute(LocalObj["zdot_obj"]-zdot_res_lv1[-1])))
            
            print("-----Terminal Angular Momentum--------")
            print("- Local Obj Angular Momentum: Lx = ", str(LocalObj["Lx_obj"]), ", Ly = ", str(LocalObj["Ly_obj"]), ", Lz = ", str(LocalObj["Lz_obj"]))
            print("- Planned Angular Momentum:   Lx = ", str(Lx_res_lv1[-1]), ", Ly = ", str(ydot_res_lv1[-1]), ", Lz = ", str(Lz_res_lv1[-1]))
            print("- Local Obj and Plan Angular Momentum Diff (Abusolute Value): Lx = ", str(np.absolute(LocalObj["Lx_obj"]-Lx_res_lv1[-1])), "Ly = ", str(np.absolute(LocalObj["Ly_obj"]-Ly_res_lv1[-1])), "Lz = ", str(np.absolute(LocalObj["Lz_obj"]-Lz_res_lv1[-1])))

            print("-----Terminal Footstep Location--------")
            print("- Local Obj Footstep Location: Px = ", str(LocalObj["Px_obj"]), ", Py = ", str(LocalObj["Py_obj"]), ", Pz = ", str(LocalObj["Pz_obj"]))
            print("- Planned Footstep Location:   Px = ", str(px_res_lv1[-1]), ", Py = ", str(py_res_lv1[-1]), ", Pz = ", str(pz_res_lv1[-1]))
            print("- Local Obj and Plan Footstep Location Diff (Abusolute Value): Px = ", str(np.absolute(LocalObj["Px_obj"]-px_res_lv1[-1])), "Py = ", str(np.absolute(LocalObj["Py_obj"]-py_res_lv1[-1])), "Pz = ", str(np.absolute(LocalObj["Pz_obj"]-pz_res_lv1[-1])))
        ...
        if var_idx_lv2:
            ...
            print('x_res: ',     np.array(x_opt[var_idx_lv2["x"][0]:var_idx_lv2["x"][1]+1]))
            print('y_res: ',     np.array(x_opt[var_idx_lv2["y"][0]:var_idx_lv2["y"][1]+1]))
            print('z_res: ',     np.array(x_opt[var_idx_lv2["z"][0]:var_idx_lv2["z"][1]+1]))
            print('xdot_res: ',  np.array(x_opt[var_idx_lv2["xdot"][0]:var_idx_lv2["xdot"][1]+1]))
            print('ydot_res: ',  np.array(x_opt[var_idx_lv2["ydot"][0]:var_idx_lv2["ydot"][1]+1]))
            print('zdot_res: ',  np.array(x_opt[var_idx_lv2["zdot"][0]:var_idx_lv2["zdot"][1]+1]))
            print('Lx_res: ',    np.array(x_opt[var_idx_lv2["Lx"][0]:var_idx_lv2["Lx"][1]+1])) if "Lx" in var_idx_lv2.keys() else None
            print('Ly_res: ',    np.array(x_opt[var_idx_lv2["Ly"][0]:var_idx_lv2["Ly"][1]+1])) if "Ly" in var_idx_lv2.keys() else None
            print('Lz_res: ',    np.array(x_opt[var_idx_lv2["Lz"][0]:var_idx_lv2["Lz"][1]+1])) if "Lz" in var_idx_lv2.keys() else None
            print('Ldotx_res: ', np.array(x_opt[var_idx_lv2["Ldotx"][0]:var_idx_lv2["Ldotx"][1]+1])) if "Ldotx" in var_idx_lv2.keys() else None
            print('Ldoty_res: ', np.array(x_opt[var_idx_lv2["Ldoty"][0]:var_idx_lv2["Ldoty"][1]+1])) if "Ldoty" in var_idx_lv2.keys() else None
            print('Ldotz_res: ', np.array(x_opt[var_idx_lv2["Ldotz"][0]:var_idx_lv2["Ldotz"][1]+1])) if "Ldotz" in var_idx_lv2.keys() else None
            print('px_res: ',    np.array(x_opt[var_idx_lv2["px"][0]:var_idx_lv2["px"][1]+1]))
            print('py_res: ',    np.array(x_opt[var_idx_lv2["py"][0]:var_idx_lv2["py"][1]+1]))
            print('pz_res: ',    np.array(x_opt[var_idx_lv2["pz"][0]:var_idx_lv2["pz"][1]+1]))
            print('Ts_res: ',    np.array(x_opt[var_idx_lv2["Ts"][0]:var_idx_lv2["Ts"][1]+1]))
            print('FR1x_res: ',  np.array(x_opt[var_idx_lv2["FR1x"][0]:var_idx_lv2["FR1x"][1]+1])) if "FR1x" in var_idx_lv2.keys() else None
            print('FR1y_res: ',  np.array(x_opt[var_idx_lv2["FR1y"][0]:var_idx_lv2["FR1y"][1]+1])) if "FR1y" in var_idx_lv2.keys() else None
            print('FR1z_res: ',  np.array(x_opt[var_idx_lv2["FR1z"][0]:var_idx_lv2["FR1z"][1]+1])) if "FR1z" in var_idx_lv2.keys() else None
...
def viewLocalObj(InitConfig = None, LocalObj = None, CurrentOptResult = None, 
    ...
    ax = drawLocaObj(LocalObj = LocalObj, InitConfig = InitConfig, fig = fig, ax = ax, LineWidth = 10)
    ...
    if groundTruthTrajPath != None:
        ...
        groundTruth_InitConfig, groundTruth_TerminalState = getInitTerminalConfig(SingleOptRes = data["SingleOptResultSavings"][roundNum], Shift_World_Frame = None)

        ax = drawInitConfig(InitConfig = groundTruth_InitConfig, fig = fig, ax = ax, color='k', LineWidth = 5)
        ax = drawTerminalConfig(TerminalConfig = groundTruth_TerminalState, InitConfig = groundTruth_InitConfig, fig = fig, ax = ax, color = 'k', LineWidth = 5)
    ...
    if CurrentOptResult != None:
        ...
        #Plot CoM Traj
        ax.plot3D(x_lv1_res,y_lv1_res,z_lv1_res,color='b', linestyle='dashed', linewidth=2, markersize=12)
        #Plot Contact Location
        if InitConfig["LeftSwingFlag"] == 1:
            ax.scatter(px_lv1_res, py_lv1_res, pz_lv1_res, c='r', marker='o', linewidth = 10)     
        if InitConfig["RightSwingFlag"] == 1:
            ax.scatter(px_lv1_res, py_lv1_res, pz_lv1_res, c='b', marker='o', linewidth = 10)   

        #Draw the Terminal (from Optimization) CoM
        ax.scatter(x_lv1_res[-1], y_lv1_res[-1], z_lv1_res[-1], c='g', marker='o', linewidth = 10)  
        ...
        if groundTruthTrajPath != None:
            ...
            ax.plot3D(x_lv1_res,y_lv1_res,z_lv1_res,color='k', linestyle='dashed', linewidth=2, markersize=12)
