...
def terrain_model_gen(terrain_name = None, fixed_inclination = None, 
    ...
    if np.max(large_slopes_index) > TotalNumSurfs:
        raise Exception("large slope index larger than the total number of surfaces")
    ...
    Sl0 = np.array([[InitContactSurf_x_max, 0.6, 0.], [-0.2, 0.6, 0.], [-0.2, 0.0, 0.], [InitContactSurf_x_max, 0.0, 0.0]])
    ...
    Sr0 = np.array([[InitContactSurf_x_max, 0, 0.], [-0.2, 0, 0.], [-0.2, -0.6, 0.], [InitContactSurf_x_max, -0.6, 0.0]])
    ...
    if (terrain_name == "flat") or (terrain_name == "stair") or (terrain_name == "single_flat") or (terrain_name == "flat_with_large_slope"):
        ...
        TerrainTypeList = ["flat","X_positive","X_negative","Y_positive","Y_negative"] #5 types of terrain
        np.random.seed()
    ...
    ContactSurfsVertice = [];   ContactSurfsHalfSpace = [];   ContactSurfsTypes = [];   ContactSurfsNames = []
    ...
    for surfNum in range(TotalNumSurfs):
        ...
        if surfNum == 0: 
            ...
            ref_z = center[2]  #ref_z decided by the central z of Sr0 (relative foot height constraints is enforced with respect to stationary foot)
            ...
            ref_y = ContactSurfsVertice[0][2][1] #ref_y decided by the P3_y of S0 ( ContactSurfsVertice[0])
            center = getCenter(ContactSurfsVertice[0])
            ref_z = center[2] #ref_z decided by the central z of S0 (relative foot height constraints is enforced with respect to stationary foot)
            ...
            if surfNum%2 == 0: #Even number of steps (Swing the Left)
                ...
                ref_x = ContactSurfsVertice[-2][0][0] #ref_x decided by the P1_x of Sn-2 (-2 in index)
                ref_y = ContactSurfsVertice[-1][0][1] #ref_y decided by the P1_y of Sn-1 (-1 in index)
                center = getCenter(ContactSurfsVertice[-1])
                ref_z = center[2] #ref_z decided by the central z of Sn-1 (relative foot height constraints is enforced with respect to stationary foot)
                ...
                ref_x = ContactSurfsVertice[-2][0][0] #ref_x decided by the P1_x of Sn-2
                ref_y = ContactSurfsVertice[-1][2][1] #ref_y decided by the P3_y of Sn-1 (ContactSurfsVertice[-1])
                center = getCenter(ContactSurfsVertice[-1])
                ref_z = center[2] #ref_z decided by the central z of Sn-1 (relative foot height constraints is enforced with respect to stationary foot)
        ...
        HalfSpace_temp = np.concatenate(convert_surface_to_inequality(surf_temp.T),axis=None)
        TangentX_temp, TangentY_temp, Norm_temp, OrientationTemp = getTerrainTagentsNormOrientation(surf_temp)
        
        ContactSurfsVertice.append(surf_temp);        ContactSurfsHalfSpace.append(HalfSpace_temp)
        ContactSurfsNames.append("S"+str(surfNum));   ContactSurfsTypes.append(terrain_pattern[surfNum])
...
def pre_designed_terrain(terrain_name = "flat", NumSteps = None, NumLookAhead = None, LargeSlopeAngle = None):
    ...
    if terrain_name == "single_large_slope_far":
        ...
        #Add the Slope
        slope = 0.325
        SlopePatch_Vertice = np.array([[1.8, 1, slope], [1.5, 1, slope], [1.5, -1.0, -slope], [1.8, -1.0, -slope]])
        ...
        #Define Initial Patches
        print("Left Init Contact Surface:")
        Sl0 = np.array([[10, 1, 0.], [-1, 1, 0.], [-1, -1, 0.], [10, -1, 0.]]) #initial patch for the left foot (for the first step only)
        LeftSurfType = getSurfaceType(Sl0) #"flat"
        print("Left Init Contact Surface Type: ",LeftSurfType)
        Sl0_TangentX, Sl0_TangentY, Sl0_Norm, Sl0_Orientation = getTerrainTagentsNormOrientation(Sl0)

        print("Right Init Contact Surface")
        Sr0 = np.array([[10, 1, 0.], [-1, 1, 0.], [-1, -1, 0.], [10, -1, 0.]]) #initial patch for the right foot (for the first step only)
        RightSurfType = getSurfaceType(Sr0) #"flat"
        print("Right Init Contact Surface Type: ",RightSurfType)
        Sr0_TangentX, Sr0_TangentY, Sr0_Norm, Sr0_Orientation = getTerrainTagentsNormOrientation(Sr0)

        FlatPatch_Vertice = np.array([[10, 1, 0.], [-1, 1, 0.], [-1, -1, 0.], [10, -1, 0.]])
