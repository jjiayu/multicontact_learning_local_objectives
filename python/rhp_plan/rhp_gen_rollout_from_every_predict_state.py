...
#NOTE: Number of Rounds need to be Updated (Consistent to the Number of iteration made by the NN tracking exp)
Nrounds = len(LoadTerrainModel["SingleOptResultSavings"])
...
for roundNum in range(len(LoadTerrainModel["SingleOptResultSavings"])): #range(Nrounds):
    print("---------------------------------------")
    print("For The ", roundNum, "Round/Step: ")
    print("---------------------------------------")

    #Get from Tracking Exp file if we have
    if roundNum <= len(LoadTerrainModel["SingleOptResultSavings"]) - 1: 
        #Load InitConfig of every round from File:
        SingleOptResult_from_File = LoadTerrainModel["SingleOptResultSavings"][roundNum]
        
        #Swing Flags
        InitConfig["LeftSwingFlag"] = SingleOptResult_from_File["LeftSwingFlag"]
        InitConfig["RightSwingFlag"] = SingleOptResult_from_File["RightSwingFlag"]

        #CoM x, y, z
        InitConfig["x_init"] = SingleOptResult_from_File["x_init"]
        InitConfig["y_init"] = SingleOptResult_from_File["y_init"]
        InitConfig["z_init"] = SingleOptResult_from_File["z_init"]

        #CoMdot x, y, z
        InitConfig["xdot_init"] = SingleOptResult_from_File["xdot_init"]
        InitConfig["ydot_init"] = SingleOptResult_from_File["ydot_init"]
        InitConfig["zdot_init"] = SingleOptResult_from_File["zdot_init"]

        #Angular Momentum
        InitConfig["Lx_init"] = SingleOptResult_from_File["Lx_init"]
        InitConfig["Ly_init"] = SingleOptResult_from_File["Ly_init"]
        InitConfig["Lz_init"] = SingleOptResult_from_File["Lz_init"]

        #Angular Momentum rate
        InitConfig["Ldotx_init"] = SingleOptResult_from_File["Ldotx_init"]
        InitConfig["Ldoty_init"] = SingleOptResult_from_File["Ldoty_init"]
        InitConfig["Ldotz_init"] = SingleOptResult_from_File["Ldotz_init"]

        #Left Init Contact
        InitConfig["PLx_init"] = SingleOptResult_from_File["PLx_init"]
        InitConfig["PLy_init"] = SingleOptResult_from_File["PLy_init"]
        InitConfig["PLz_init"] = SingleOptResult_from_File["PLz_init"]
        InitConfig["PL_init_TangentX"] = SingleOptResult_from_File["PL_init_TangentX"]
        InitConfig["PL_init_TangentY"] = SingleOptResult_from_File["PL_init_TangentY"]
        InitConfig["PL_init_Norm"] = SingleOptResult_from_File["PL_init_Norm"]
        InitConfig["LeftInitSurf"] = SingleOptResult_from_File["LeftInitSurf"]
        InitConfig["LeftInitSurfOrientation"] = SingleOptResult_from_File["LeftInitSurfOrientation"]

        #Right Init Contact
        InitConfig["PRx_init"] = SingleOptResult_from_File["PRx_init"]
        InitConfig["PRy_init"] = SingleOptResult_from_File["PRy_init"]
        InitConfig["PRz_init"] = SingleOptResult_from_File["PRz_init"]
        InitConfig["PR_init_TangentX"] = SingleOptResult_from_File["PR_init_TangentX"]
        InitConfig["PR_init_TangentY"] = SingleOptResult_from_File["PR_init_TangentY"]
        InitConfig["PR_init_Norm"] = SingleOptResult_from_File["PR_init_Norm"]
        InitConfig["RightInitSurf"] = SingleOptResult_from_File["RightInitSurf"]
        InitConfig["RightInitSurfOrientation"] = SingleOptResult_from_File["RightInitSurfOrientation"]
    ...
    #   Get Terrain Patches (Get from Terrain Model)
