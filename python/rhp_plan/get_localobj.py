...
def getLocalobj(Mode = None, refTrajFile = None, roundNum = None, \
    ...
    if Mode == "fromFile":
        ...
        InitConfig, TerminalConfig = getInitTerminalConfig(SingleOptRes = optResult, Shift_World_Frame = shift_world_frame)

        #Span the variables, NOTE:Remember to Scale things Back
        localobj["x_obj"]    = TerminalConfig["x_end"];        localobj["y_obj"]    = TerminalConfig["y_end"]         
        localobj["z_obj"]    = TerminalConfig["z_end"]

        localobj["xdot_obj"] = TerminalConfig["xdot_end"];     localobj["ydot_obj"] = TerminalConfig["ydot_end"]      
        ...
        if ContactParameterizationType == "3DPoints":
            localobj["Px_obj"]   = TerminalConfig["Px"];           localobj["Py_obj"]   = TerminalConfig["Py"]            
            ...
            P_localobj = ConvexCombination_to_Point3D(Coef = coefs, ContactSurf = FirstPatch)
            ...
            terrain_x_vec = np.array([FirstPatch[0]]).T - np.array([FirstPatch[1]]).T
            terrain_y_vec = np.array([FirstPatch[1]]).T - np.array([FirstPatch[2]]).T
        ...
        shiftedInitConfig, shiftedTerminalConfig = shiftInitTerminalConfig_to_LocalFrame(InitConfig = InitConfig, TerminalConfig = None, Local_Frame_Selection = shift_world_frame)
        ...
        if shift_world_frame == None: #No transformation to local frames, then full input vector
            ...
            if (shiftedInitConfig["LeftSwingFlag"] == 1) and (shiftedInitConfig["RightSwingFlag"] == 0): #Swing Left Foot
                contact_before_swing_X = shiftedInitConfig["PLx_init"];     contact_before_swing_Y = shiftedInitConfig["PLy_init"]
                contact_before_swing_Z = shiftedInitConfig["PLz_init"]
            elif (shiftedInitConfig["LeftSwingFlag"] == 0) and (shiftedInitConfig["RightSwingFlag"] == 1): #Swing Right Foot
                contact_before_swing_X = shiftedInitConfig["PRx_init"];     contact_before_swing_Y = shiftedInitConfig["PRy_init"]
        ...
        if DataSetSettings["PreProcessMode"] == "Standarization" or DataSetSettings["PreProcessMode"] == "MaxAbs":
            print("Transfrom Input from Original Form into: ", DataSetSettings["PreProcessMode"])
        ...
        if DataSetSettings["PreProcessMode"] == "Standarization" or DataSetSettings["PreProcessMode"] == "MaxAbs":
            print("Transfrom Output from: ", DataSetSettings["PreProcessMode"], ", back to Original Form")
            PredictVector = DataSetSettings["Scaler_Y"].inverse_transform(PredictVector)
        elif DataSetSettings["PreProcessMode"] == "OriginalForm":
            print("Data Represented in Original Form, Scale with a constant")
        ...
        if dataset["PreProcessMode"] == "Standarization" or dataset["PreProcessMode"] == "MaxAbs":
            print("Transfrom Input from Original Form into: ", dataset["PreProcessMode"])
        ...
        if dataset["PreProcessMode"] == "Standarization" or dataset["PreProcessMode"] == "MaxAbs":
            print("Transfrom Output from: ", dataset["PreProcessMode"], ", back to Original Form")
            PredictVector = dataset["Scaler_Y"].inverse_transform(PredictVector)
        elif dataset["PreProcessMode"] == "OriginalForm":
            print("Data Represented in Original Form, No need to transform")
        else:
            print("Unknown Pre Processing Model")
...
def add_Noise_to_Localobj(LocalObj = None, InitConfig = None, noiseLevel = 0.003):
    ...
    P_original = np.concatenate([LocalObj["Px_obj"],LocalObj["Py_obj"],LocalObj["Pz_obj"]], axis = None)
    P_shifted = P_original + shiftValue_X*InitConfig["SurfTangentsX"][0] + shiftValue_Y*InitConfig["SurfTangentsY"][0]

    #   Get 4 vertices of the shifted contact location
    P1_shifted = P_shifted + 0.11*InitConfig["SurfTangentsX"][0] + 0.06*InitConfig["SurfTangentsY"][0]
    P2_shifted = P_shifted + 0.11*InitConfig["SurfTangentsX"][0] - 0.06*InitConfig["SurfTangentsY"][0]
    P3_shifted = P_shifted - 0.11*InitConfig["SurfTangentsX"][0] + 0.06*InitConfig["SurfTangentsY"][0]
    P4_shifted = P_shifted - 0.11*InitConfig["SurfTangentsX"][0] - 0.06*InitConfig["SurfTangentsY"][0]
    ...
    LocalObj["Px_obj"] = P_shifted[0]; LocalObj["Py_obj"] = P_shifted[1];  LocalObj["Pz_obj"] = P_shifted[2]
