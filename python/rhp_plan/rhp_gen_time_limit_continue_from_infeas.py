...
if NumLookAhead == 1: #Single Step NLP
    ...
    solver, DecisionVars_lb, DecisionVars_ub, glb, gub, var_index = ocp_solver_build(FirstLevel = "NLP_SingleStep", SecondLevel = None, TotalNumSteps = NumLookAhead, \
                                                                                     ...
                                                                                     max_compute_time = 0.8)
    #Solver for 1-2 step
    solver_1_and_2, DecisionVars_lb, DecisionVars_ub, glb, gub, var_index = ocp_solver_build(FirstLevel = "NLP_SingleStep", SecondLevel = None, TotalNumSteps = NumLookAhead, \
                                                                                             LocalObjTrackingType = LocalObjSettings["local_obj_tracking_type"], \
                                                                                             N_knots_local = N_knots_per_phase, robot_mass = RobotMass, \
                                                                                             PhaseDurationLimits=phase_duration_limits,\
                                                                                             max_compute_time = 200.0)
    ...
    solver, DecisionVars_lb, DecisionVars_ub, glb, gub, var_index = ocp_solver_build(FirstLevel = "NLP_SingleStep", SecondLevel = "NLP_SecondLevel", \
                                                                                     ...
                                                                                     max_compute_time = 0.8)
    #Solver for 1-2 step
    solver_1_and_2, DecisionVars_lb, DecisionVars_ub, glb, gub, var_index = ocp_solver_build(FirstLevel = "NLP_SingleStep", SecondLevel = "NLP_SecondLevel", \
                                                                                             TotalNumSteps = NumLookAhead, LocalObjTrackingType = None, \
                                                                                             N_knots_local = N_knots_per_phase, robot_mass = RobotMass,\
                                                                                             PhaseDurationLimits=phase_duration_limits,\
                                                                                             max_compute_time = 200.0)
...
for roundNum in range(Nrounds):
    ...
    if roundNum == 0 or roundNum == 1:
        res = solver_1_and_2(x0=DecisionVars_init_list[0], p = ParaList, lbx = DecisionVars_lb, ubx = DecisionVars_ub, lbg = glb, ubg = gub)
    else:
    ...
    #Get solver stats
    if roundNum == 0 or roundNum == 1:
        solverStats = solver_1_and_2.stats()
    else:
        solverStats = solver.stats()
    print(solverStats["return_status"])

    if solverStats["success"] == True:
        solver_success_flag = True
    elif solverStats["success"] == False:
        if solverStats["return_status"] == "KTR_RC_TIME_LIMIT_INFEAS" or solverStats["return_status"] == "KTR_RC_TIME_LIMIT_FEAS":
            solver_success_flag = True
        else:
            solver_success_flag = False
    else:
        raise Exception("Unknown Success Situation")

    if solver_success_flag==True:
        print("Round ", roundNum, solverStats["success"])
        ...
        print("Total Program Time: ", round(solverStats["t_proc_total"],4))
    ...
    elif solver_success_flag == False: #and not (solverStats["return_status"] == "KTR_RC_TIME_LIMIT_FEAS"):
